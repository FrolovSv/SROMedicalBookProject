...
public class MedicalBook extends MainClassProgect<MedicalBook>{
    ...
    public String getEmployeeName() {return EmployeeName == null ? "" : EmployeeName;    }
    public String getEmployeeLastName() {return EmployeeLastName==null ? "" : EmployeeLastName;    }
    public String getEmployeePatronymic() { return  EmployeePatronymic == null ? "" : EmployeePatronymic;     }
    public String getEmployeeUsGroupName() {return EmployeeUsGroupName == null ? "" : EmployeeUsGroupName ;    }
    public String getEmployeeUsDepartmentName() {return EmployeeUsDepartmentName == null ? "" : EmployeeUsDepartmentName;}
    public String getEmployeeUsPositionName() {return EmployeeUsPositionName == null ? "" : EmployeeUsPositionName;}
    public String getMainEmployeeName() {return MainEmployeeName == null ? "" : MainEmployeeName;     }
    public String getMainEmployeeLastName() {return MainEmployeeLastName == null ? "" : MainEmployeeLastName;}
    public String getMainEmployeePatronymic() {return MainEmployeePatronymic == null ? "" : MainEmployeePatronymic;}
    public String getMainEmployeeUsGroupName() {return MainEmployeeUsGroupName == null ? "" : MainEmployeeUsGroupName;}
    public String getMainEmployeeUsDepartmentName() { return MainEmployeeUsDepartmentName == null ? "" : MainEmployeeUsDepartmentName;}
    public String getMainEmployeeUsPositionName() {return MainEmployeeUsPositionName == null ? "" : MainEmployeeUsPositionName;}
    public Integer getEmployeeId() {return EmployeeId == null ? 0 : EmployeeId;   }
...
//          Удаленная	String	false	false		
        ...
        else EmplName= this.getSurnameInitials(getEmployeeLastName(), getEmployeeName(), getEmployeePatronymic());
        ...
        DataForTable[9] = checkValidMedicalBook()? "Норма" : "Просрочено";
        DataForTable[10] = sb;        
        return DataForTable;        
    }
    ...
    public String getQuerySelectTable(Integer Limit) {
        String str = "SELECT "
                    ...
                    + "MB.Hepatitis_A, MB.Hepatitis_A2, MB.Diphtheria, MB.Measles, MB.Hepatitis_B, "//5
                    + "MB.Hepatitis_B2, MB.Hepatitis_B3, MB.ValidationForYear, MB.Note, MB.RegionId, "//5
                    + "MB.Rubella, MB.hepatitisAvaccine, MB.Measles_2, MB.isDeleted, "
                    + "Emp.LastName, Emp.name, Emp.Patronymic, "
                    + "UsDepEmp.name, UsGrEmp.name, UsPosEmp.name, "
                    + "MainEmp.name, MainEmp.LastName, MainEmp.Patronymic, "
                    + "Inst.Name, Emp.InstitutionId "//4
                    + "FROM `MedicalBook` MB "
...
//        + "MB.NumMedicalBook, MB.EmployeeId, MB.BorneDiseases, MB.Therapist, MB.Otolaryngologist, MB.Dentist, " //5
//        + "MB.ExpertInNarcology, MB.Psychiatrist, MB.Dermatovenerologist, MB.Fluorography, MB.Helminths, "//5
//        + "MB.IntestinalInfection, MB.TyphoidFever, MB.Staphylococcus, MB.Validation, MB.Shigellvak, "//5
//        + "MB.Hepatitis_A, MB.Hepatitis_A2, MB.Diphtheria, MB.Measles, MB.Hepatitis_B, "//5
//        + "MB.Hepatitis_B2, MB.Hepatitis_B3, MB.ValidationForYear, MB.Note, MB.RegionId, "//5
...
//        + "Inst.Name "//4
        ...
        MedicalBook.setEmploymentDate(rs.getDate(42));
        return MedicalBook;
    }
    ...
    public Boolean checkValidMedicalBook(){
        Boolean Valid = false;
        try{
...
//            проработка иммунитета к АДСМ
            ValidationSet.add(BorneDiseases.contains(EnumBorneDiseases.Diphtheria) ? true : getDiphtheria().after(CurrentDayMinus10Year.getTime()));//АДСМ
            
//            проработка иммунитета к КРАСНУХЕ
            if (BorneDiseases.contains(EnumBorneDiseases.Rubella)){
                ValidationSet.add(true);
            } else {
                if (getEmployeeBerthday().after(CurrentDayMinus25Year.getTime())) { // не более 25 лет, то привика не нужна
                    ValidationSet.add(true);
                } else {
                    ValidationSet.add(!getRubella().equals(new Date(0))); // не равно нулевому значению
                }
            }
//            проработка иммунитета к КОРИ     
            if (getBorneDiseases().contains(EnumBorneDiseases.Measles)) {
                ValidationSet.add(true); // getMeasles первая вакцина 
                ValidationSet.add(true); // getMeasles_2 вторая вакцина 
            } else {
                if (getEmployeeBerthday().before(CurrentDayMinus55Year.getTime())) { // менее 55 лет, то прививка не нужна
                    ValidationSet.add(true); // getMeasles первая вакцина 
                    ValidationSet.add(true); // getMeasles_2 вторая вакцина 
                } else {
                    if (getMeasles().equals(new Date(0)) && getMeasles_2().equals(new Date(0))) {  // если нет прививок от кори
                        ValidationSet.add(false); // getMeasles первая вакцина 
                        ValidationSet.add(false); // getMeasles_2 вторая вакцина 
                    }else if(!getMeasles().equals(new Date(0)) && getMeasles_2().equals(new Date(0))) { // если нет второй прививки от кори
                        ValidationSet.add(getMeasles().after(CurrentDayMinus3Month.getTime())); // не более 3 месяцев от текущей
                    }else if(!getMeasles().equals(new Date(0)) && !getMeasles_2().equals(new Date(0))) { // если есть обе прививки от кори
                        ValidationSet.add(getMeasles().before(DateMeasles_2Minus3Month.getTime())); // не менее 3 месяца до 2ой привики
                    }
                }
            }
//              проработка иммунитета к Гепатиту А     
            if (getBorneDiseases().contains(EnumBorneDiseases.Hepatitis_A)) {
                ValidationSet.add(true); // getHepatitis_A первая вакцина 
                ValidationSet.add(true); // getHepatitis_A2 вторая вакцина 
            } else {
...
//                проработка иммунитета к ГепатитуВ     
            if (getBorneDiseases().contains(EnumBorneDiseases.Hepatitis_B)) {
                ValidationSet.add(true); // getHepatitis_B первая вакцина 
                ValidationSet.add(true); // getHepatitis_B2 вторая вакцина 
                ValidationSet.add(true); // getHepatitis_B3 третья вакцина 
            } else {
                if (getEmployeeBerthday().before(CurrentDayMinus55Year.getTime())) { // менее 55 лет, то прививка не нужна
                    ValidationSet.add(true); // getHepatitis_B первая вакцина 
                    ValidationSet.add(true); // getHepatitis_B2 вторая вакцина 
